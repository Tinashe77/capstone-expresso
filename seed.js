...
db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='Employee'", (error, table) => {
  ...
  if (table) {
    ...
    db.serialize(function() {
      let employeeId;
      db.run("INSERT INTO Employee (name, position, wage) VALUES ('TINASHE', 'Manager', 10)");
      db.run("INSERT INTO Employee (name, position, wage) VALUES ('CLAYTON', 'Employee', 15)", function(error) {
        if (error) {
          throw new Error(error);
        }
  ...
  if (table) {
    db.serialize(function() {
      ...
      db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='MenuItem'", (error, table) => {
        ...
        if (table) {
          db.run(`INSERT INTO MenuItem (name, description, inventory, price, menu_id) VALUES ('Lasagne', 'My favourite dish', 13, 5, ${menuId})`);
          db.run(`INSERT INTO MenuItem (name, description, inventory, price, menu_id) VALUES ('pizza', 'The meal that i dont  like', 65, 34, ${menuId})`);
        }
      });
    });
  }
